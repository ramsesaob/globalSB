...
const Pagos_internos = () => {
  ...
  const calculateMontoUsd = (montoBsf, tasa) => {
    const parseInputToNumber = (inputValue) => {
      if (!inputValue) return 0;
      const cleanedValue = inputValue.toString().replace(/[^0-9,.]/g, "").replace(',', '.');
      const number = parseFloat(cleanedValue);
      return isNaN(number) ? 0 : number;
    };

    const numMontoBsf = parseInputToNumber(montoBsf);
    const numTasa = parseInputToNumber(tasa);

    return (numMontoBsf >= 0 && numTasa > 0) 
      ? (numMontoBsf / numTasa).toFixed(4) 
      : "0.0000";
  };
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const checkResponse = await fetch(`${apiBaseUrl}/pagos/index.json`);
...
const savePayment = async () => {
  try {
    const response = await fetch(`${apiBaseUrl}/pagos/guardar.json`, {
      method: 'POST',
      headers: {
  ...
  const checkDuplicatePayments = async () => {
   ...
   const formProveedor = formData.proveedor.trim().toLowerCase();
    ...
    if (
      ...
      try {
        const response = await fetch(`${apiBaseUrl}/pagos/index.json`);
  ...
  return (
    <div className="container">
      <h3 className="text-center pt-3">
        Programacion de Pagos<i className="mx-2 bx bx-credit-card"> </i>  
      </h3>
           ...
           {/* Tasa */}
            ...
            {/* Botón de enviar + advertencia */}
              <div className="d-flex flex-column align-items-center mt-3">
                ...
                >
                  {isSubmitting ? 'Guardando...' : 'Guardar solicitud de pago'}
                </button>
